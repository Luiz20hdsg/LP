...
fun teval (ConI _) _ = IntT
    ...
    | teval (Prim1(operator, exp)) (env:plcType env) =
        let
            val expType = teval exp env
        in
            case operator of "!" => if expType = BoolT then BoolT else raise UnknownType
    ...
    | teval (Prim2(operator, exp1, exp2)) (env:plcType env) =
        let
            val exp1Type = teval exp1 env
            val exp2Type = teval exp2 env
        in
            case operator of "&&" => if exp1Type = BoolT andalso exp2Type = BoolT then BoolT else raise UnknownType
            | "::" =>
                let in
